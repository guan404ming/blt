...
Our test set consists of 500 popular songs scraped from a commercial music platform, spanning pop, ballad, and rock genres from the 2000s to 2020s, with roughly equal coverage of English and Chinese tracks.
Each test case uses 4--20 lines from a song, with lines filtered to contain only the source language via automatic language detection.
Due to copyright restrictions, we do not distribute the lyrics themselves; our open-source release (Apache-2.0) includes the full translation framework, IPA tool suite, LangGraph agent definitions, and evaluation scripts as a PyPI package (\texttt{blt-toolkit}), enabling reproduction on any user-supplied lyrics.
...
We report SER and SCRE as defined in \S\ref{sec:eval}.
We do not report ARI because our 4--20-line excerpts frequently contain only one or two rhyme classes, making the clustering comparison degenerate: when most lines fall into a single class, ARI is dominated by chance agreement and becomes unstable.
A meaningful ARI evaluation requires excerpts with at least two rhyme classes of two or more lines each; we estimate fewer than 30\% of our test cases meet this criterion, insufficient for reliable aggregate statistics.
...
ARI is theoretically well-suited for rhyme evaluation but is unstable on our short excerpts: with only 4--20 lines, most excerpts have sparse rhyme structure (often a single dominant class), and ARI degenerates.
Future work should either (1) evaluate on full songs with richer rhyme patterns, restricting ARI reporting to excerpts with at least two rhyme classes of two or more lines each, or (2) adopt a lighter-weight proxy such as pairwise rhyme precision/recall on line pairs that share a rhyme label in the source.
Adding a dedicated rhyme refinement phase is also a natural next step.
...
Future work will focus on evaluation across diverse language pairs and LLM architectures, human judgments of singability, and integration with singing voice synthesis systems \citep{ren-etal-2020-deepsinger} for end-to-end singable translation.
