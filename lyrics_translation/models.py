...
class CoTTranslation(BaseModel):
    ...
    def _save_txt(self, output_path: Path) -> None:
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            ...
            f.write("【Step 1: 意義分析】\n")
            f.write(f"{self.meaning_analysis}\n\n")

            f.write("【Step 2: 約束分析】\n")
            for key, value in self.constraint_analysis.items():
                f.write(f"  - {key}: {value}\n")
            f.write("\n")

            f.write("【Step 3: 關鍵詞構思】\n")
            for keyword in self.keyword_ideas:
                f.write(f"  - {keyword}\n")
            f.write("\n")
            ...
            f.write("【Step 5: 自我驗證】\n")
            for key, value in self.self_verification.items():
                f.write(f"  - {key}: {value}\n")
            f.write("\n")
