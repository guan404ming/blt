...
class ConstraintValidator:
    ...
    def _validate_rhyme(
        ...
        # 計算準確率
        total_checks = sum(len(indices) - 1 for indices in rhyme_groups.values() if len(indices) > 1)
        accuracy = (total_checks - mismatches) / total_checks if total_checks > 0 else 1.0
    ...
    def generate_feedback(self, validation_result: ValidationResult) -> str:
        ...
        boundary_errors = [e for e in validation_result.errors if e['type'] == 'boundary']
        ...
        if length_errors:
            ...
            for error in length_errors_sorted:
                ...
                diff = actual - expected if isinstance(actual, int) and isinstance(expected, int) else '?'
                diff_str = f"({diff:+d})" if isinstance(diff, int) else ""
                feedback_parts.append(f"  第 {line_num} 行: 本次 {actual} 音節 → 目標 {expected} 音節 {diff_str * -1}")
