...
def generate_report(agent_file, base_file, report_path):
    ...
    agent_results = [r for r in agent_data.get('results', []) if r.get('method') == 'agent']
    base_results = [r for r in base_data.get('results', []) if r.get('method') == 'baseline']
    ...
    if len(agent_results) < total_expected:
        ...
        report_lines.append(f"- **Agent**: Only {len(agent_results)}/{total_expected} tests completed")
        report_lines.append("")
    ...
    report_lines.append(f"| **SER** (↓) | {agent_metrics['ser']:.4f} | {base_metrics['ser']:.4f} | {ser_diff:+.4f} | {ser_winner} |")
    
    scre_diff = agent_metrics['scre'] - base_metrics['scre']
    scre_winner = "🟢 Agent" if agent_metrics['scre'] < base_metrics['scre'] else "🔴 Base"
    if agent_metrics['scre'] > 0:
        scre_ratio = base_metrics['scre'] / agent_metrics['scre']
    else:
        scre_ratio = 0
    report_lines.append(f"| **SCRE** (↓) | {agent_metrics['scre']:.4f} | {base_metrics['scre']:.4f} | {scre_diff:+.4f} ({scre_ratio:.1f}x) | {scre_winner} |")
    
    ari_diff = agent_metrics['ari'] - base_metrics['ari']
    ari_winner = "🟢 Agent" if agent_metrics['ari'] > base_metrics['ari'] else "🔴 Base"
    report_lines.append(f"| **ARI** (↑) | {agent_metrics['ari']:.4f} | {base_metrics['ari']:.4f} | {ari_diff:+.4f} | {ari_winner} |")
    ...
    if base_metrics['avg_time_seconds'] > 0:
        ...
        speedup = 0
    time_winner = "🟢 Agent" if agent_metrics['avg_time_seconds'] < base_metrics['avg_time_seconds'] else "🔴 Base"
    report_lines.append(f"| **Time(s)** (↓) | {agent_metrics['avg_time_seconds']:.2f} | {base_metrics['avg_time_seconds']:.2f} | {time_diff:+.2f}s ({speedup:.1f}x) | {time_winner} |")
    ...
    if len(agent_results) < total_expected:
        report_lines.append(f"⚠️ **Incomplete**: Agent only completed {len(agent_results)}/{total_expected} tests")
        report_lines.append("")
    ...
    if base_metrics['scre'] > 3:
        ...
        report_lines.append("**Recommendation**: Use Agent (Better quality, acceptable speed)")
    elif agent_wins >= 2:
        report_lines.append("**Recommendation**: Use Agent (Better quality despite slow speed)")
    elif agent_wins == 1:
