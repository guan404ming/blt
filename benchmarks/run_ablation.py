...
def run_single_phase(
    ...
    hf_tag = "_hf" if "hf" in str(test_suite_path) else ""

    cmd = [
        sys.executable, "-m", "benchmarks.run_experiment",
    ...
    print(f"\n{'─' * 50}")
    print(f"Running: {PHASE_LABELS[phases]}")
...
def print_summary(all_results: dict):
    ...
    print(f"{'=' * 75}")
    print(f"{'Phase':<35} {'SER ↓':>10} {'SCRE ↓':>10} {'ARI ↑':>10} {'Time':>10}")
    print(f"{'─' * 75}")

    for phases in [1, 2, 3]:
        if phases not in all_results:
            print(f"{PHASE_LABELS[phases]:<35} {'(missing)':>10}")
            continue

        data = all_results[phases]
        metrics = data.get("agent_avg_metrics", {})
        if not metrics:
            print(f"{PHASE_LABELS[phases]:<35} {'(no data)':>10}")
            continue

        def fmt(val, decimals=4):
            return f"{val:.{decimals}f}" if isinstance(val, (int, float)) else "N/A"

        ser = fmt(metrics.get("ser"))
        scre = fmt(metrics.get("scre"))
        ari = fmt(metrics.get("ari"))
        time_s = f"{metrics.get('avg_time_seconds', 0):.1f}s" if isinstance(metrics.get("avg_time_seconds"), (int, float)) else "N/A"
        print(f"{PHASE_LABELS[phases]:<35} {ser:>10} {scre:>10} {ari:>10} {time_s:>10}")


def save_ablation_summary(all_results: dict, source_lang: str, target_lang: str, model: str, output_dir: Path) -> Path:
...
def main():
    ...
    parser.add_argument("--samples", type=int, default=5, help="Number of test samples (default: 5)")
    parser.add_argument("--max-lines", type=int, default=5, help="Max lines per test case (default: 5)")
    parser.add_argument("--model", type=str, default="qwen3:30b-a3b-instruct-2507-q4_K_M", help="Ollama model name")
    parser.add_argument("--data-dir", type=str, default="benchmarks/data", help="Lyrics data directory")
    parser.add_argument("--hf-data", type=str, default=None, help="Path to extracted HF datasets/ directory")
    ...
    if args.hf_data:
        ...
        source_file_map = {"cmn": "cmn_lyrics.json", "en-us": "en_lyrics.json", "ja": "ja_lyrics.json"}
        source_file = Path(args.data_dir) / source_file_map.get(args.source_lang, f"{args.source_lang}_lyrics.json")
        if not source_file.exists():
    ...
    # Save test suite for subprocess reuse
    ...
    for phases in [1, 2, 3]:
        ...
        if returncode != 0:
            print(f"Phase {phases} exited with code {returncode}, continuing...")
    ...
    if all_results:
        ...
        save_ablation_summary(all_results, args.source_lang, args.target_lang, args.model, output_dir)
    else:
